...
+ (NSString *)dayFromTimeStamp:(NSTimeInterval)timeStamp
{
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:timeStamp/1000];
    return [self dayFromWeekday:date];
}
...
- (NSDate *)offsetMinutes:(int)minutes {
    return [NSDate offsetMinutes:minutes fromDate:self];
}

+ (NSDate *)offsetMinutes:(int)numMinutes fromDate:(NSDate *)fromDate {
    if (fromDate == nil) {
        return nil;
    }
...
- (NSDate *)dateToMonday {
    NSCalendar *calendar = [[NSCalendar alloc]
        ...
        // 获取这周的第一天日期
        NSDateComponents *firstComponents = [calendar components:unitNewFlag fromDate:self];
        [firstComponents setDay:day - countDays];
        NSDate *firstDate = [calendar dateFromComponents:firstComponents];
        return firstDate;
    //    NSLog(@"date === %@", [firstDate stringWithFormat:@"yyyy-MM-dd HH:mm"]);
